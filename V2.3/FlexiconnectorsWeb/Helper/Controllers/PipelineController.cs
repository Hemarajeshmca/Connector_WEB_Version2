...
namespace FlexicodeConnectors.Controllers
{
    public class PipelineController : Controller
    {
        ...
        [HttpPost]
        public async Task<bool> ValidateFile(string file_Paths, string sheetName, string filename)
        {
            bool flag = false;
            var dummyfile_name = "";
            var file_extension = "";
        ...
        [HttpPost]
        public async Task<JsonResult> Updateexistingpplapiheader([FromBody] pplApiHeader objpplapicsvheader)
        {
            var results = "";
            var json = JsonConvert.SerializeObject(objpplapicsvheader);
            var stringContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");
            ConnectorApi _api = new ConnectorApi(_configuration);
            HttpClient client = _api.Initial();
        ...
        //getAPIResponse
        [HttpGet]
        public async Task<JsonResult> getAPIResponse(string url)
        {
            //  List<pplSourceCsv> pplsrccsv = new List<pplSourceCsv>();
            var results = "";
            string post_data = "";
            ...
            ConnectorApi _api = new ConnectorApi(_configuration);
            HttpClient client = _api.Initial();
            HttpResponseMessage res = await client.GetAsync("Pipeline/GetPODetails");
            if (res.IsSuccessStatusCode)
            {
                results = res.Content.ReadAsStringAsync().Result;
                JObject jsonObj = JObject.Parse(results);

                // Generate empty structure
                JObject emptyStructure = GenerateSchema(jsonObj);

                // Convert back to JSON
                string jsonSchema = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
